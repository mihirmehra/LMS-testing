...
export function LeadImportModal({ open, onOpenChange, onImportComplete }: LeadImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    formData.append('csvFile', file); // 'csvFile' must match the key expected by your backend route

    try {
      console.log("CLIENT: Sending POST request to /api/leads/import...");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file to add multiple leads to your system.
          </DialogDescription>
        </DialogHeader>

...
export function AddLeadModal({ open, onOpenChange, onAddLead, existingLeads }: AddLeadModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert receivedDate string to Date if it's a string
      let parsedReceivedDate: Date = new Date()
      if (formData.receivedDate) {
        if (typeof formData.receivedDate === "string") {
          parsedReceivedDate = parseDDMMYYYY(formData.receivedDate) || new Date()
        } else if (formData.receivedDate instanceof Date) {
          parsedReceivedDate = formData.receivedDate
        }
      }

...
// The functions from push-devices are now async due to MongoDB operations
import { getDeviceRegistrations, updateDeviceRegistration, deleteDeviceRegistration } from '@/lib/dev-db/push-devices';
...
// FIX 1: Change this to a real, valid email address
const VAPID_SUBJECT = 'mailto:your_actual_email@example.com'; // <--- IMPORTANT: Update this!
...
export async function POST(request: NextRequest) {
  try {
    ...
    // FIX 2: Await the call to getDeviceRegistrations() as it is now async
    const allDevices = await getDeviceRegistrations(); // <--- AWAIT THIS CALL
    const userDevices = allDevices.filter(device => device.userId === userId && device.isActive);
    ...
    // Payload structure (already updated in previous steps, looks good)
    const payload = JSON.stringify({
      title,
      body: message,
      icon: icon || '/favicon.png',
      badge: '/favicon.png',
      tag: tag || 'realestate-crm',
      vibrate: [200, 100, 200],
      data: {
        url: url || '/',
        ...data,
    ...
    const sendPromises = userDevices.map(async (device) => {
      try {
        ...
        await updateDeviceRegistration(device); // <--- AWAIT THIS CALL
        ...
        if (error.statusCode === 404 || error.statusCode === 410) {
          ...
          // FIX 4: Await updateDeviceRegistration() as it is now async
          await updateDeviceRegistration(device); // <--- AWAIT THIS CALL
          // If you want to immediately delete:
          // await deleteDeviceRegistration(device.id); // <--- AWAIT THIS CALL if you uncomment
        } else {
          // Other errors
